...
public final class ExhortApi implements Api {
    ...
    private enum TokenProvider {
        ...
        String getHeaderName() {
            return String.format(
                    "ex-%s-token", this.toString().replace("_", "-").toLowerCase());
        }
    ...
    ExhortApi(final HttpClient client) {
        //    // temp system property - as long as prod exhort url not implemented the multi-source v4 endpoint, this
        ...
        if (System.getProperty("EXHORT_DEV_MODE") == null) {
            try {
                ...
                LOG.info(String.format(
                        "Error loading config.properties , fallback to set default property DEV MODE = false, Error message = %s",
                        e.getMessage()));
                System.setProperty("EXHORT_DEV_MODE", "false");
            }
        }
    ...
    public String getExhortUrl() {
        ...
        if (debugLoggingIsNeeded()) {
            LOG.info(String.format(
                    "EXHORT_DEV_MODE=%s,DEV_EXHORT_BACKEND_URL=%s, Chosen Backend URL=%s , DEFAULT_ENDPOINT_DEV=%s , DEFAULT_ENDPOINT=%s",
                ...
                .thenApply(resp -> {
                    ...
                    if (resp.statusCode() == 200) {
                        ...
                        LOG.severe(String.format(
                                "failed to invoke stackAnalysisMixed for getting the html and json reports, Http Response Status=%s , received message from server= %s ",
        ...
        return this.client
                ...
                .thenApply(httpResponse -> {
                    ...
                    if (httpResponse.statusCode() != 200) {
                        LOG.severe(String.format(
                                "failed to invoke stackAnalysis for getting the html report, Http Response Status=%s , received message from server= %s ",
                ...
                .exceptionally(exception -> {
                    LOG.severe(String.format(
                            "failed to invoke stackAnalysis for getting the html report, received message= %s ",
    ...
    public CompletableFuture<AnalysisReport> stackAnalysis(final String manifestFile) throws IOException {
        ...
        return this.client
                ...
                .thenApply(
                        response -> getAnalysisReportFromResponse(response, "StackAnalysis", "json", exClientTraceId))
                .exceptionally(exception -> {
                    LOG.severe(String.format(
                            "failed to invoke stackAnalysis for getting the json report, received message= %s ",
                            exception.getMessage()));
                    //        LOG.log(System.Logger.Level.ERROR, "Exception Entity", exception);
    ...
    private AnalysisReport getAnalysisReportFromResponse(
        ...
        if (response.statusCode() == 200) {
            if (debugLoggingIsNeeded()) {
                LOG.info(String.format(
                        "Response body received from exhort server : %s %s", System.lineSeparator(), response.body()));
            }
            commonHookAfterExhortResponse();
            try {
            ...
            LOG.severe(String.format(
                    "failed to invoke %s for getting the %s report, Http Response Status=%s , received message from server= %s ",
    ...
    private static void logExhortRequestId(HttpResponse response) {
        Optional<String> headerExRequestId = response.headers().allValues(EXHORT_REQUEST_ID_HEADER_NAME).stream()
                .findFirst();
        headerExRequestId.ifPresent(value -> LOG.info(String.format(
                "Unique Identifier associated with this request ( Received from Exhort Backend ) - ex-request-id= : %s",
                value)));
    }
    ...
    @Override
    public CompletableFuture<AnalysisReport> componentAnalysis(final String manifestType, final byte[] manifestContent)
            throws IOException {
        String exClientTraceId = commonHookBeginning(false);
    ...
    private void commonHookAfterProviderCreatedSbomAndBeforeExhort() {
        if (debugLoggingIsNeeded()) {
            ...
            LOG.info(String.format(
                    "Time took to create sbom file to be sent to exhort backend, in ms : %s, in seconds: %s",
    ...
    private CompletableFuture<AnalysisReport> getAnalysisReportForComponent(
        ...
        return this.client
                ...
                //      .thenApply(HttpResponse::body)
                .thenApply(response ->
                        getAnalysisReportFromResponse(response, "Component Analysis", "json", exClientTraceId))
                .exceptionally(exception -> {
                    LOG.severe(String.format(
                            "failed to invoke Component Analysis for getting the json report, received message= %s ",
                            exception.getMessage()));
                    //        LOG.log(System.Logger.Level.ERROR, "Exception Entity", exception);
                    return new AnalysisReport();
                });
    ...
    @Override
    public CompletableFuture<Map<ImageRef, AnalysisReport>> imageAnalysis(final Set<ImageRef> imageRefs)
            throws IOException {
                ...
                .map(imageRef -> {
                    try {
                        return new AbstractMap.SimpleEntry<>(
                                imageRef.getPackageURL().canonicalize(), ImageUtils.generateImageSBOM(imageRef));
                ...
                return reports.entrySet().stream()
                        .collect(Collectors.toMap(
                                e -> {
                                    try {
                                        return new ImageRef(
                                                new PackageURL(e.getKey().toString()));
    ...
    <H, T> CompletableFuture<T> performBatchAnalysis(
        ...
        var content = new Provider.Content(
                mapper.writeValueAsString(sboms).getBytes(StandardCharsets.UTF_8), Api.CYCLONEDX_MEDIA_TYPE);
        commonHookAfterProviderCreatedSbomAndBeforeExhort();
        return this.client
                .sendAsync(this.buildRequest(content, uri, mediaType, analysisName), responseBodyHandler)
                .thenApply(response -> getBatchAnalysisReportsFromResponse(
