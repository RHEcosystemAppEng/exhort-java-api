...
class PythonControllerRealEnvTest extends ExhortTest {
    ...
    //    }
    //    // in var args, must override type default method' void.class in argumentMatcher interface in order to let
    ...
    void get_Dependencies_With_Match_Manifest_Versions(boolean MatchManifestVersionsEnabled) {
        Set<String> expectedSetOfPackages = Set.of(
                "click",
                "flask",
                "importlib-metadata",
                "zipp",
        ...
        if (MatchManifestVersionsEnabled) {
            RuntimeException runtimeException = assertThrows(
                    ...
                    "Expected getDependencies/2 to throw RuntimeException, due to version mismatch, but it didn't.");
            operationsMockedStatic.close();
            assertTrue(
                    runtimeException
                            .getMessage()
                            .contains(
                                    "Can't continue with analysis - versions mismatch for dependency name=click, manifest version=8.0.5, installed Version=8.0.4"));
        } else {
            ...
            Set<String> expectedSetOfPackagesLC = expectedSetOfPackages.stream()
                    .map(packageName -> packageName.replace("_", "-"))
                    .map(String::toLowerCase)
                    .collect(Collectors.toSet());

            Set<String> actualSetOfPackagesLC = actualSetOfPackages.stream()
                    .map(packageName -> packageName.replace("_", "-"))
                    .map(String::toLowerCase)
                    .collect(Collectors.toSet());
    ...
    void get_Dependencies_from_Cyclic_Tree() {
        MockedStatic<Operations> operationsMockedStatic = Mockito.mockStatic(Operations.class);
        //    ArgumentMatcher<String[]> matchCommandPipFreeze = command -> Arrays.stream(command).anyMatch(word ->
